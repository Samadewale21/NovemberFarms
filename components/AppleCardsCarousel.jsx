...
export const Carousel = ({ items, initialScroll = 0 }) => {
  ...
  useEffect(() => {
    if (carouselRef.current) {
      // Start in the middle
      carouselRef.current.scrollLeft = initialScroll + items.length * 300;
      checkScrollability();
    }
  ...
  const scrollLeft = () => {
    if (carouselRef.current) {
      ...
      // Check if we've reached the beginning of the duplicated items
      if (carouselRef.current.scrollLeft <= 0) {
        setTimeout(() => {
          carouselRef.current.scrollLeft = items.length * 300; // Jump to the last set
        }, 300);
      }
    }
  };
  ...
  const scrollRight = () => {
    if (carouselRef.current) {
      ...
      if (carouselRef.current.scrollLeft >= (items.length * 3 - 1) * 300) {
        setTimeout(() => {
          carouselRef.current.scrollLeft = items.length * 300; // Jump to the first set
        }, 300);
      }
    }
  };
  ...
  const handleCardClose = (index) => {
    ...
    const scrollPosition = (cardWidth + gap) * (adjustedIndex + 1) + items.length * 300;
  ...
  return (
    <CarouselContext.Provider value={{ onCardClose: handleCardClose, currentIndex }}>
      <div className="relative w-full">
        <div
          className="flex w-full overflow-x-auto overscroll-x-auto py-10 md:py-20 scroll-smooth [scrollbar-width:none]"
          ref={carouselRef}
